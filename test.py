...
class MainWindow(QMainWindow):
    ...
    def start_camera(self):
        # Check if camera is not in ActiveState
        if self.camera.state() != 2:
            # start the camera
            self.camera.start()
            ...
            self.alert('Camera not running')


    def stop_camera(self):
        # Check if camera is in ActiveState
        if self.camera.state() == 2:
            self.camera.stop()
        else:
            self.alert('Camera not running')
